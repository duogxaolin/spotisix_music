...
$keywords = xss($_GET['keywords']);
$keywords = $duogxaolin->to_slug($keywords);
$query = "SELECT * FROM `songs`    ";
if(isset($_GET['type'])){
    $type = xss($_GET['type']);
    if($type == '3'){
        $query .= "WHERE `SongSlug` LIKE '%$keywords%' or `SongName` LIKE '%$keywords%' ";
    }else if($type == '2'){
     $query .= "WHERE `ArtistID` IN (SELECT `ArtistID` FROM `artists` WHERE `ArtistSlug` LIKE '%$keywords%' or `ArtistName` LIKE '%$keywords%')  ";
    }
}else{
    $type = '';
    $query .= " WHERE `SongSlug` LIKE '%$keywords%' OR `ArtistID` IN (SELECT `ArtistID` FROM `artists` WHERE `ArtistSlug` LIKE '%$keywords%' or `ArtistName` LIKE '%$keywords%') ";
}
$query .= "ORDER BY `SongID` DESC";
$list = $duogxaolin->get_list($query);
            ...
            <form action="/search" class="d-flex align-items-sm-center align-items-baseline gap-md-6 gap-sm-3 gap-1">
                <div class="select-category-area">
                    <select name="type"  class="select-category">
                        <option value="1" <?= ($type== '' ? 'selected' : '') ?> >Tất cả</option>
                        <option value="3" <?= ($type== '3' ? 'selected' : '') ?>>Theo tên bài hát</option>
                        <option value="2" <?= ($type== '2' ? 'selected' : '') ?>>Theo Nghệ sĩ</option>
                        <option value="" disabled>Lời bài hát(comming soon)</option>
                    </select>
                </div>
                <div class="input-area p-0 w-100">
                    <input type="text" class="py-1" name="keywords" value="<?=$keywords?>" placeholder="Tìm bài hát, nghệ sĩ...">
                    <button class="p-0">
        ...
        <div class="row g-6">
            <div class="col-xl-12">
                <div class="blog-details-content-wrapper d-grid gap-6 p-xxl-6 p-lg-4 p-3 rounded bgc-3">
                    <h4 class="fw-semibold">Kết quả tìm kiếm </h4>
